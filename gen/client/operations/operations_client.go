...
type ClientService interface {
	...
	ListEmployee(params *ListEmployeeParams) (*ListEmployeeOK, error)
...
/*
  ListEmployee get all the employees
*/
func (a *Client) ListEmployee(params *ListEmployeeParams) (*ListEmployeeOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewListEmployeeParams()
	}
	...
	result, err := a.transport.Submit(&runtime.ClientOperation{
		...
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &ListEmployeeReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
	...
	success, ok := result.(*ListEmployeeOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for ListEmployee: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
